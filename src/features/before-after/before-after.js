...
async function loadBeforeAfter() {
  const sec = document.getElementById('ba-section');
  if (!sec) return;
  ...
  try {
    ...
    const [u1, u2] = await Promise.all([
      signedPhoto(first.photo_path),
      signedPhoto(last.photo_path),
    ]);
    if (!u1 || !u2) return;
    document.getElementById('ba-before-img').src = u1;
    document.getElementById('ba-after-img').src = u2;
    const rg = document.getElementById('ba-range');
    if (rg) rg.value = 50;
    baSlide(50);
    const fmt = (d) => {
      const p = d.split('-');
    ...
    document.getElementById('ba-caption').textContent =
      '+' + days + ' jour' + (days > 1 ? 's' : '') + ' entre les deux 🌸';
