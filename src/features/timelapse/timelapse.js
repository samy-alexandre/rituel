...
async function openTimelapse() {
  ...
  if (rows.length < 2) {
    showToast('Il faut au moins 2 photos 🌸');
    return;
  }
  tlPhotos = [];
  for (const r of rows) {
    const url = await signedPhoto(r.photo_path);
    if (url) tlPhotos.push({ url, date: r.date });
  }
  if (tlPhotos.length < 2) {
    showToast('Photos indisponibles');
    return;
  }
  const rg = document.getElementById('tl-range');
  rg.max = String(tlPhotos.length - 1);
  rg.value = '0';
  rg.oninput = (ev) => {
    tlStop();
    tlShow(parseInt(ev.target.value, 10));
  };
...
function tlShow(i) {
  ...
  const dt = document.getElementById('tl-date');
  if (dt) dt.textContent = fmtDateLong(p.date) + '  ·  ' + (i + 1) + '/' + tlPhotos.length;
  const rg = document.getElementById('tl-range');
  if (rg) rg.value = String(i);
}
function tlStart() {
  tlPlaying = true;
...
function tlToggle() {
  tlPlaying ? tlStop() : tlStart();
}
function closeTimelapse() {
  tlStop();
  document.getElementById('timelapse-sheet').classList.remove('open');
}
