...
async function refreshPushBtn() {
  const b = document.getElementById('push-btn'),
    st = document.getElementById('push-status');
  if (!b) return;
  if (!('serviceWorker' in navigator) || !('PushManager' in window)) {
    b.style.display = 'none';
    if (st)
      st.textContent =
        "Non supporté sur ce navigateur (sur iPhone : installe d'abord l'app sur l'écran d'accueil).";
    return;
  }
  try {
...
async function togglePush() {
  try {
    ...
    if (perm !== 'granted') {
      showToast('Autorise les notifications pour continuer 🌸');
      return;
    }
    const r = await fetch('/api/push');
    const d = await r.json();
    if (!d || !d.publicKey) {
      showToast('Push pas encore configuré côté serveur 🔧');
      return;
    }
    sub = await reg.pushManager.subscribe({
      userVisibleOnly: true,
      applicationServerKey: urlB64ToU8(d.publicKey),
    });
    const j = sub.toJSON();
    await fetch('/api/push', {
      method: 'POST',
