...
async function loadHabits() {
  ...
  const hs = habits || [];
  if (!hs.length) {
    list.innerHTML = `<div style="text-align:center;color:var(--muted);font-size:13px;padding:14px;line-height:1.5;">Aucune habitude pour l'instant.<br>Suis ce que tu veux : eau, sport, masque… 🌸</div>`;
    return;
  }
  ...
  const byHabit = {};
  (logs || []).forEach((l) => {
    (byHabit[l.habit_id] = byHabit[l.habit_id] || new Set()).add(l.date);
  });
...
async function toggleHabitToday(id, done) {
  ...
  if (done) {
    ...
    await sb.from('habit_logs').insert({ user_id: currentUser.id, habit_id: id, date: today });
  }
  loadHabits();
}
function renderHabitEmoji() {
  document.getElementById('habit-emoji-picker').innerHTML = HABIT_EMOJIS.map(
    (e) =>
      `<button type="button" class="emoji-opt${e === hfEmoji ? ' selected' : ''}" onclick="pickHabitEmoji('${e}')">${e}</button>`
  ).join('');
}
function pickHabitEmoji(e) {
  hfEmoji = e;
  renderHabitEmoji();
}
...
async function openHabitForm(id) {
  ...
  document.getElementById('habit-sheet-title').textContent = id
    ? "Modifier l'habitude"
    : 'Nouvelle habitude';
  renderHabitEmoji();
  ...
  if (id) {
    ...
    if (h) {
      document.getElementById('hf-nom').value = h.nom || '';
      hfEmoji = h.emoji || '💧';
      renderHabitEmoji();
    }
  }
...
async function saveHabit() {
  ...
  if (!nom) {
    document.getElementById('hf-error').textContent = 'Donne un nom à ton habitude.';
    return;
  }
...
async function deleteHabit() {
  ...
  if (
    !(await cmAsk({
      ...
      texte: 'Son historique complet sera effacé.',
      ok: 'Supprimer',
      annuler: 'Annuler',
      danger: true,
    }))
...
async function renderDayHabits(ds) {
  const host = document.getElementById('dd-habits');
  ...
  if (!hs.length) {
    host.innerHTML =
      '<div style="color:var(--muted);font-size:13px;">Aucune habitude. Crée-en sur l\'accueil 🌸</div>';
    return;
  }
  const { data: logs } = await sb
    .from('habit_logs')
  ...
  host.innerHTML = hs
    .map(
      (h) =>
        `<div class="rem-row" style="padding:10px 0;"><div class="rem-label">${h.emoji || '✨'} ${escapeHtml(h.nom || '')}</div><button class="habit-toggle${doneSet.has(h.id) ? ' done' : ''}" onclick="toggleHabitDate('${h.id}','${ds}',this)">${doneSet.has(h.id) ? '✓' : ''}</button></div>`
    )
    .join('');
}
async function toggleHabitDate(id, ds, btn) {
  ...
  if (isDone) {
    ...
    await sb.from('habit_logs').insert({ user_id: currentUser.id, habit_id: id, date: ds });
