...
async function openRituelEditor(id, presetMoment) {
  ...
  const { data: prods } = await sb
    .from('products')
    .select('id,nom,emoji,effets,categorie,date_peremption')
  ...
  if (id) {
    ...
    rituelDraft = r
      ? {
          ...
          produits: Array.isArray(r.produits) ? r.produits.slice() : [],
        }
      : { id: null, nom: '', moment: 'matin', produits: [] };
...
function closeRituelSheet() {
  document.getElementById('rituel-sheet').classList.remove('open');
}
...
function rdCollectRappels() {
  ...
  if (m === 'matin' || m === 'both') {
    obj.matin = document.getElementById('rd-h-matin').value || '08:00';
  }
  if (m === 'soir' || m === 'both') {
    obj.soir = document.getElementById('rd-h-soir').value || '21:00';
  }
...
function rdPickHebdoDay(d) {
  rdHebdoDay = d;
  document
    .querySelectorAll('.rd-hday')
    .forEach((b) => b.classList.toggle('on', parseInt(b.dataset.hday, 10) === d));
...
function rdSyncRemTimes() {
  const tm = document.getElementById('rd-time-matin'),
    ts = document.getElementById('rd-time-soir');
  if (!tm || !ts) return;
  const m = rituelDraft ? rituelDraft.moment : 'matin';
...
function rdSetMoment(m) {
  ...
  const hd = document.getElementById('rd-hebdo-day');
  if (hd) hd.style.display = m === 'hebdo' ? 'block' : 'none';
  ...
  if (document.getElementById('rd-rem-on') && document.getElementById('rd-rem-on').checked)
    rdSyncRemTimes();
}
function rdRemove(id) {
  rituelDraft.produits = rituelDraft.produits.filter((x) => x !== id);
  renderRdProducts();
}
function renderRdProducts() {
  const list = document.getElementById('rd-products');
  if (!list) return;
  if (!rituelDraft.produits.length) {
    list.innerHTML = '';
    return;
  }
  list.innerHTML = rituelDraft.produits
    .map((id, i) => {
      const p = rdProductMap[id];
      if (!p) return '';
...
function rdActives(p) {
  ...
  if (/\baha\b|\bbha\b|\bpha\b|glycoliqu|salicyliqu|lactiqu|mand[ée]liqu|peeling|exfoli/.test(s))
    set.add('acids');
...
function detectConflicts(prods) {
  const all = new Set();
  prods.forEach((p) => rdActives(p).forEach((a) => all.add(a)));
  const m = [];
  if (all.has('retinoid') && all.has('acids'))
    m.push(
      'Rétinoïde + acides exfoliants : souvent trop irritant dans le même rituel. Beaucoup préfèrent les espacer (un soir sur deux).'
    );
  if (all.has('retinoid') && all.has('vitc'))
    m.push(
      'Rétinoïde + vitamine C : on les sépare souvent · vitamine C le matin, rétinoïde le soir.'
    );
  if (all.has('retinoid') && all.has('bpo'))
    m.push(
      "Rétinoïde + peroxyde de benzoyle : peuvent s'irriter ou se neutraliser selon la formule · souvent mieux d'alterner."
    );
  if (all.has('acids') && all.has('vitc'))
    m.push(
      'Acides exfoliants + vitamine C : combo parfois irritant · beaucoup préfèrent les espacer.'
    );
  return m;
}
function renderRdConflicts() {
  const box = document.getElementById('rd-conflicts');
  if (!box) return;
  const prods = ((rituelDraft && rituelDraft.produits) || [])
    .map((id) => rdProductMap[id])
    .filter(Boolean);
  ...
  box.innerHTML =
    '<div style="background:#FBF1E6;border:1px solid #EBD9BE;border-radius:14px;padding:12px 14px;font-size:12.5px;color:#7A5A2E;line-height:1.55;margin-bottom:4px;">💡 <b>Petit conseil</b><br>' +
    msgs.map(escapeHtml).join('<br>') +
    '<br><button class="btn-soft" style="padding:7px 14px;font-size:11.5px;margin-top:8px;" onclick="closeRituelSheet();navTo(\'chat\')">En parler à Léa →</button><div style="margin-top:6px;color:#A98A5E;font-size:11px;">Ça dépend de ta peau et des formules · ce n\'est pas un avis médical.</div></div>';
}
function renumberRd() {
  const list = document.getElementById('rd-products');
  if (!list) return;
  [...list.querySelectorAll('.rd-card')].forEach((c, i) => {
    const s = c.querySelector('.rd-step');
    if (s) s.textContent = 'ÉTAPE ' + (i + 1);
  });
}
function initRituelDnd() {
  ...
  list.querySelectorAll('.rd-handle').forEach((h) => {
    h.onpointerdown = (e) => {
      e.preventDefault();
      const dragEl = h.closest('.rd-card');
      if (!dragEl) return;
...
function setRdSort(s) {
  rdSort = s;
  document
    .querySelectorAll('[data-rdsort]')
    .forEach((b) => b.classList.toggle('sel', b.dataset.rdsort === s));
  renderRdPickerList();
}
function toggleRdCatDD() {
  document.getElementById('rd-cat-menu').classList.toggle('open');
}
function pickRdCat(cat, label) {
  rdCatFilter = cat;
  document
    .querySelectorAll('#rd-cat-menu .cat-dd-opt')
    .forEach((b) => b.classList.toggle('sel', b.dataset.cat === cat));
...
function renderRdPickerList() {
  ...
  if (rdCatFilter !== 'all') {
    ids = ids.filter((id) => {
      const p = rdProductMap[id];
      return p && (p.categorie || 'autre') === rdCatFilter;
    });
  }
  // tri
  const favs = typeof favSet === 'function' ? favSet() : new Set();
  ids.sort(function (a, b) {
    const pa = rdProductMap[a],
      pb = rdProductMap[b];
    if (rdSort === 'alpha') {
      return (pa.nom || '').localeCompare(pb.nom || '', 'fr', { sensitivity: 'base' });
    }
    if (rdSort === 'perem') {
      const da = pa.date_peremption || '',
        db = pb.date_peremption || '';
      if (!da && !db) return 0;
      if (!da) return 1;
      if (!db) return -1;
  ...
  if (!ids.length) {
    list.innerHTML =
      '<div style="color:var(--muted);font-size:13px;padding:8px 2px;line-height:1.5;">Aucun produit dans cette vue 🌸</div>';
    return;
  }
  list.innerHTML = ids
    .map((id) => {
      const p = rdProductMap[id];
      const sel = rdPickerSel.has(id);
      const nom = (p.nom || '').split(' · ')[0];
      return (
        '<button class="rd-pick' +
        (sel ? ' picked' : '') +
        '" data-id="' +
        id +
        '" onclick="rdTogglePick(\'' +
        id +
        '\',this)" style="display:flex;align-items:center;gap:12px;width:100%;padding:10px 12px;border-radius:14px;border:1.5px solid ' +
        (sel ? 'var(--accent)' : 'var(--line)') +
        ';background:var(--surface);cursor:pointer;text-align:left;">' +
        '<span style="font-size:20px;">' +
        (p.emoji || '🧴') +
        '</span>' +
        '<span style="flex:1;font-size:14px;color:var(--ink);">' +
        nom +
        '</span>' +
        '<span style="width:22px;height:22px;border-radius:50%;border:1.5px solid ' +
        (sel ? 'var(--accent)' : 'var(--line)') +
        ';background:' +
...
function openRituelHelp() {
  document.getElementById('rituel-help-overlay').style.display = 'flex';
}
function closeRituelHelp() {
  document.getElementById('rituel-help-overlay').style.display = 'none';
}
...
async function openRdPicker() {
  ...
  setTimeout(function () {
    document
      .querySelectorAll('[data-rdsort]')
      .forEach((b) => b.classList.toggle('sel', b.dataset.rdsort === 'fav'));
    const cb = document.getElementById('rd-cat-btn');
    if (cb) {
      cb.innerHTML = 'Catégorie <span style="font-size:8px;opacity:0.6;">▼</span>';
      cb.classList.remove('active-filter');
    }
    document
      .querySelectorAll('#rd-cat-menu .cat-dd-opt')
      .forEach((b) => b.classList.toggle('sel', b.dataset.cat === 'all'));
  }, 0);
  renderRdPickerList();
  document.getElementById('rd-picker').classList.add('open');
}
function rdTogglePick(id, btn) {
  if (rdPickerSel.has(id)) {
    rdPickerSel.delete(id);
  } else {
    rdPickerSel.add(id);
  }
  renderRdPickerList();
}
function closeRdPicker() {
  document.getElementById('rd-picker').classList.remove('open');
}
function confirmRdPicker() {
  rdPickerSel.forEach((id) => {
    if (!rituelDraft.produits.includes(id)) rituelDraft.produits.push(id);
  });
  closeRdPicker();
  renderRdProducts();
}
async function saveRituel() {
  ...
  const _autoNom =
    rituelDraft.moment === 'matin'
      ? 'Rituel du matin'
      : rituelDraft.moment === 'soir'
        ? 'Rituel du soir'
        : rituelDraft.moment === 'hebdo'
          ? 'Soins de la semaine'
          : 'Mon rituel';
  ...
  try {
    const { data: cp } = await sb
      .from('products')
      .select('id,categorie')
      .in('id', rituelDraft.produits);
    const rk = {};
    (cp || []).forEach((p) => (rk[p.id] = p.categorie in CAT_RANK ? CAT_RANK[p.categorie] : 99));
    const before = rituelDraft.produits.join(',');
    rituelDraft.produits = [...rituelDraft.produits]
      .map((id, i) => ({ id, i }))
      .sort((x, y) => (rk[x.id] ?? 99) - (rk[y.id] ?? 99) || x.i - y.i)
      .map((x) => x.id);
    if (rituelDraft.produits.join(',') !== before)
      showToast('Ordre ajusté selon les catégories ✨');
  } catch (e) {}
  try {
    const { data: oth } = await sb
      .from('rituels')
      .select('id,moment,actif')
      .eq('user_id', currentUser.id);
    ...
    const offIds = (oth || [])
      .filter(
        (r) =>
          ...
          r.moment !== 'hebdo' &&
          rituelDraft.moment !== 'hebdo' &&
          ((myM && (r.moment === 'matin' || r.moment === 'both')) ||
            (myS && (r.moment === 'soir' || r.moment === 'both')))
      )
      .map((r) => r.id);
  ...
  if (rituelDraft.id) {
    await sb
      ...
      .update({
        nom,
        moment: rituelDraft.moment,
        produits: rituelDraft.produits,
        actif: true,
        rappels: rdCollectRappels(),
      })
      .eq('id', rituelDraft.id);
  } else {
    const { data: existing } = await sb.from('rituels').select('id').eq('user_id', currentUser.id);
    const position = existing ? existing.length : 0;
    const { data: _ins } = await sb
      .from('rituels')
      .insert({
