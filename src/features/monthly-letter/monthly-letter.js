...
function lettreSeen() {
  try {
    return !!localStorage.getItem(
      'rituel_lettreseen_' + (currentUser ? currentUser.id : 'x') + '_' + lettreYM()
    );
  } catch (e) {
    return false;
  }
}
function lettreArchive() {
  try {
    return JSON.parse(localStorage.getItem('rituel_lettres_' + currentUser.id) || '[]');
  } catch (e) {
    return [];
  }
}
function maybeShowLettreCta() {
  const el = document.getElementById('lettre-cta');
  if (!el) return;
  if (!currentUser) {
  ...
  const t = document.getElementById('lettre-cta-title');
...
async function fetchLettre() {
      ...
      messages: [
        {
          role: 'user',
          content:
            "(Message automatique de l'app, ce n'est pas une vraie question de la personne.) Écris ta petite lettre du mois : un bilan chaleureux en 4 à 6 phrases d'après mes données (série, éclat, habitudes, peau), avec un encouragement doux pour le mois qui commence. Pas de question à la fin. Termine par une signature courte avec ton prénom.",
        },
      ],
      profile: ctx,
  ...
  if (txt) {
    try {
      localStorage.setItem(bilanKey(), txt);
    } catch (e) {}
  }
  return txt;
}
async function openLettre(arch) {
  ...
  if (typeof arch === 'string') {
    body.innerHTML = formatMessage(arch);
    return;
  }
  body.innerHTML =
    '<div style="color:var(--muted);font-size:13px;text-align:center;padding:14px 0;">' +
    coachName() +
    ' écrit ta lettre… ✍️</div>';
  const txt = await fetchLettre();
  if (!txt) {
    body.innerHTML =
      '<div style="color:var(--muted);font-size:13px;text-align:center;padding:14px 0;">Oups, réessaie dans un instant 🌿</div>';
    return;
  }
  body.innerHTML = formatMessage(txt);
  try {
    const a = lettreArchive();
    if (!a.some((x) => x.k === lettreYM())) {
      a.unshift({ k: lettreYM(), t: txt });
      localStorage.setItem('rituel_lettres_' + currentUser.id, JSON.stringify(a));
    }
  } catch (e) {}
}
function closeLettre() {
  ...
  try {
    localStorage.setItem('rituel_lettreseen_' + currentUser.id + '_' + lettreYM(), '1');
  } catch (e) {}
  maybeShowLettreCta();
}
function openLettreAt(i) {
  const a = lettreArchive();
  if (a[i]) openLettre(a[i].t);
}
  ...
  host.innerHTML = a.length
    ? a
        .map(function (x, i) {
          const p = x.k.split('-');
          return (
            '<button class="btn-soft" style="display:block;width:100%;text-align:left;margin-bottom:8px;padding:12px 14px;border-radius:14px;" onclick="openLettreAt(' +
            i +
            ')">💌 Lettre de ' +
            M[parseInt(p[1], 10) - 1] +
            ' ' +
...
function closeLettresList() {
  document.getElementById('lettres-sheet').classList.remove('open');
}
