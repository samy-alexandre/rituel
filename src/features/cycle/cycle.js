...
function cyclePhaseInfo() {
  ...
  if (day <= 5) {
    phase = 'menstruelle';
    emoji = '🌙';
    tip = "Peau souvent plus sensible · douceur et hydratation aujourd'hui.";
  } else if (day < ovu - 1) {
    phase = 'folliculaire';
    emoji = '🌱';
    tip = 'Ta peau est en forme · bon moment pour tes actifs.';
  } else if (day <= ovu + 1) {
    phase = 'ovulation';
    emoji = '☀️';
    tip = 'Éclat naturel au rendez-vous ✨';
  } else {
    phase = 'lutéale';
    emoji = '🍂';
    tip = 'Imperfections possibles · nettoyage doux et patience.';
  }
  return { day, phase, emoji, tip, len };
}
function loadCycleCard() {
  const sec = document.getElementById('cycle-section'),
    card = document.getElementById('cycle-card');
  if (!sec || !card) return;
  ...
  if (i.stale) {
    card.innerHTML =
      '<div style="display:flex;align-items:center;gap:14px;background:var(--surface);border:1px solid var(--line);border-radius:18px;padding:14px 16px;box-shadow:var(--shadow);cursor:pointer;" onclick="openCycleSheet()"><div style="font-size:24px;flex-shrink:0;">🌸</div><div style="flex:1;min-width:0;"><span class="eyebrow">SUIVI DE CYCLE</span><div style="font-size:13px;color:var(--ink-soft);line-height:1.45;margin-top:3px;">Ça fait un moment · mets à jour la date de tes dernières règles pour des repères justes.</div></div></div>';
    sec.style.display = 'block';
    return;
  }
  card.innerHTML =
    '<div style="display:flex;align-items:center;gap:14px;background:var(--surface);border:1px solid var(--line);border-radius:18px;padding:11px 14px;box-shadow:var(--shadow);" onclick="openCycleSheet()">' +
    '<div style="font-size:24px;flex-shrink:0;">' +
    i.emoji +
    '</div>' +
    '<div style="flex:1;min-width:0;"><div style="display:flex;align-items:center;gap:8px;"><span class="eyebrow">CYCLE · JOUR ' +
    i.day +
    '</span><span style="font-size:11px;font-weight:600;color:var(--accent-deep);text-transform:capitalize;">' +
    i.phase +
    '</span></div>' +
    '<div style="font-size:13px;color:var(--ink-soft);line-height:1.45;margin-top:3px;">' +
    i.tip +
    '</div></div>' +
    '</div>';
  sec.style.display = 'block';
}
function updateCycleMenuMeta() {
  const m = document.getElementById('menu-cycle-meta');
  if (m) m.textContent = state.cycleEnabled ? 'activé' : '…';
}
function cySetEnabled(on) {
  cyTmp.enabled = on;
  const a = document.getElementById('cy-on'),
    b = document.getElementById('cy-off'),
    body = document.getElementById('cy-body');
  if (a) a.classList.toggle('selected', on);
  if (b) b.classList.toggle('selected', !on);
  if (body) body.style.display = on ? 'block' : 'none';
}
function cyToday() {
  const el = document.getElementById('cy-start');
  if (el) {
    const t = todayStr();
    el.setAttribute('data-iso', t);
    el.value = dpFmtDisplay(t);
  }
}
function openCycleSheet() {
  cyTmp = { enabled: !!state.cycleEnabled };
  const st = document.getElementById('cy-start');
  if (st) {
    const _v = state.cycleLastStart || '';
    st.setAttribute('data-iso', _v);
    st.value = _v ? dpFmtDisplay(_v) : '';
  }
  const ln = document.getElementById('cy-length');
  if (ln) ln.value = state.cycleLength || 28;
  cySetEnabled(cyTmp.enabled);
  document.getElementById('cycle-sheet').classList.add('open');
}
function closeCycleSheet() {
  document.getElementById('cycle-sheet').classList.remove('open');
}
async function saveCycle() {
  ...
  state.cycleLength = Math.max(
    20,
    Math.min(45, parseInt(document.getElementById('cy-length').value, 10) || 28)
  );
  if (currentUser) {
    try {
      await sb
        .from('profiles')
        .update({
          cycle_enabled: state.cycleEnabled,
          cycle_last_start: state.cycleLastStart,
  ...
  updateCycleMenuMeta();
  loadCycleCard();
  if (state.cycleEnabled && !state.cycleLastStart)
    showToast('Pense à noter le 1er jour de tes règles 🌸');
