...
function closeLeaPop() {
  const p = document.getElementById('lea-pop');
  if (p) p.classList.remove('show');
  const m = document.getElementById('lea-pop-menu');
  if (m) m.classList.remove('open');
}
function leaPopMenu(ev) {
...
function toggleLeaPop() {
  const b = document.getElementById('lea-bubble');
  if (!b) return;
  ...
  p.innerHTML =
    '<div class="lea-pop-head"><span class="lea-pop-av">' +
    nm.charAt(0).toUpperCase() +
    '</span><span class="lea-pop-name">' +
    nm +
    '</span>' +
  ...
  let top = r.top - ph - 10;
  if (top < 64) top = Math.min(r.bottom + 10, window.innerHeight - ph - 14);
