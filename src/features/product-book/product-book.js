...
const PB_GRAD = {
  demaquillant: ['#F7E7E1', '#E9CDC4'],
  nettoyant: ['#E9F1E7', '#CFE0CB'],
  toner: ['#E7EDF1', '#CDD9E1'],
  serum: ['#F8EDDA', '#EDD6B0'],
  yeux: ['#EEE8F1', '#D8CDE1'],
  creme: ['#F0EBE3', '#DFD4C4'],
  spf: ['#FCF0D6', '#F2DCA0'],
  masque: ['#F5E6EE', '#E4C6D6'],
  cible: ['#F7E4DE', '#EBC6BB'],
  autre: ['#EFEBE3', '#DED5C6'],
};
...
function pbDates(p) {
  ...
  if (p.date_ouverture)
    rows +=
      '<div class="pb-date-row"><span>Ouvert le</span><b>' + fmt(p.date_ouverture) + '</b></div>';
  if (p.date_peremption) {
    ...
    let txt;
    if (days < 0) {
      const dp = Math.abs(days);
      txt =
        '⚠️ Périmé ' +
        (dp === 0 ? "aujourd'hui" : dp === 1 ? 'depuis 1 jour' : 'depuis ' + dp + ' jours');
    } else if (days === 0) {
      txt = "⚠️ Périme aujourd'hui";
    } else {
      txt =
        'Périme le ' + fmt(p.date_peremption) + ' · ' + (days === 1 ? '1 jour' : days + ' jours');
    }
    rows +=
      '<div class="pb-date-row"><span>Fraîcheur</span><span class="pb-badge" style="background:' +
      col[0] +
      ';color:' +
      col[1] +
      ';">' +
...
async function openProductBook(id, minimal) {
  ...
  if (!p) return;
  const el = document.getElementById('product-book');
  if (!el) return;
  ...
  if (p.photo_path) {
    ...
    visual = url
      ? '<img src="' + url + '" alt="">'
      : '<div class="pb-halo"></div><div class="pb-emoji">' + (p.emoji || '🧴') + '</div>';
  } else
    visual = '<div class="pb-halo"></div><div class="pb-emoji">' + (p.emoji || '🧴') + '</div>';
  const isFav = favSet().has(p.id);
  let chips = '';
  if (!minimal && p.prix != null && p.prix !== '')
    chips += '<span class="pb-chip price">' + p.prix + '€</span>';
  ...
  el.innerHTML =
    ...
    (minimal
      ...
      : '<button class="pb-fav' +
        (isFav ? ' on' : '') +
        '" id="pb-fav-btn" onclick="pbToggleFav(\'' +
        p.id +
        '\')">' +
        (isFav ? '★' : '☆') +
        '</button>') +
    '<div class="pb-visual" style="background:linear-gradient(150deg,' +
    g[0] +
    ',' +
    g[1] +
    ');">' +
    visual +
    '</div>' +
    '<div class="pb-card"><div class="pb-cat">' +
    catL +
    '</div><div class="pb-name">' +
    name +
    '</div>' +
    (brand ? '<div class="pb-brand">' + brand + '</div>' : '') +
    desc +
    (minimal
      ? ''
      : (chips ? '<div class="pb-chips">' + chips + '</div>' : '') +
        pbDates(p) +
        '<div class="pb-actions"><button class="pb-edit" onclick="pbEdit(\'' +
        p.id +
        '\')">Modifier</button><button class="pb-del" onclick="pbDelete(\'' +
        p.id +
...
function pbToggleFav(id) {
  const s = favSet();
  if (s.has(id)) s.delete(id);
  else s.add(id);
  favSave(s);
  const b = document.getElementById('pb-fav-btn');
  if (b) {
    b.textContent = s.has(id) ? '★' : '☆';
    b.classList.toggle('on', s.has(id));
  }
}
