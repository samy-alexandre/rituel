...
const THEMES = {
  ...
  bleu: {
    ...
    swatch: ['#5B89BE', '#9BBEE0', '#E5EEF8'],
  },
};
function currentTheme() {
  try {
    return localStorage.getItem('rituel_theme') || 'terracotta';
  } catch (e) {
    return 'terracotta';
  }
}
function applyTheme(key) {
  ...
  try {
    localStorage.setItem('rituel_theme', key);
  } catch (e) {}
}
function openThemeSheet() {
  const cur = currentTheme();
  document.getElementById('theme-list').innerHTML = Object.entries(THEMES)
    .map(
      ([k, t]) =>
        `<button class="theme-opt${k === cur ? ' selected' : ''}" onclick="pickTheme('${k}')"><div class="theme-swatch">${t.swatch.map((c) => `<span style="background:${c};"></span>`).join('')}</div><div class="theme-name">${t.name}</div><div class="theme-check"></div></button>`
    )
    .join('');
