...
async function cnCardHtml(p, favs) {
  ...
  return (
    ...
    '<div class="cn-thumb" style="background:' +
    tint +
    ';">' +
    thumb +
    '</div>' +
...
async function cnRenderChaptered(products, favs) {
  ...
  for (const cat of CATS) {
    ...
    html +=
      '<div class="cn-chapter"><span class="cn-chapter-ic">' +
      cat[1] +
      '</span><span class="cn-chapter-t">' +
      cat[2] +
      '</span><span class="cn-chapter-n">' +
      items.length +
      '</span><span class="cn-chapter-rule"></span></div>';
