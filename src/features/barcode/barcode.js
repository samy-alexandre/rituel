...
async function openBarcodeSheet() {
  ...
  const manual = document.getElementById('bc-manual');
  ...
  if (
    ...
    status.textContent =
      "Le scan caméra n'est pas supporté sur ce navigateur · tape le code à la main 👇";
    return;
  }
  try {
    ...
    bcTimer = setInterval(async () => {
      ...
      try {
        ...
        if (codes && codes.length && codes[0].rawValue) {
          ...
          bcLookup(code);
        } else if (Date.now() - bcT0 > 7000) {
          status.textContent = 'Rapproche-toi du code et stabilise 🤳 (ou tape-le en dessous)';
        }
      } catch (err) {}
      bcBusy = false;
    }, 300);
...
function closeBarcodeSheet() {
  stopBcCamera();
  document.getElementById('barcode-sheet').classList.remove('open');
}
function bcLookupManual() {
  const v = (document.getElementById('bc-manual').value || '').trim();
  if (v) bcLookup(v);
}
async function bcFetchProduct(base, code) {
  try {
    ...
    const r = await fetch(base + encodeURIComponent(code) + '.json', { signal: ctl.signal });
    clearTimeout(t);
    const d = await r.json();
    if (d && d.status === 1 && d.product) return d.product;
  } catch (e) {}
  return null;
}
async function bcLookup(code) {
  const status = document.getElementById('bc-status');
  const g = document.getElementById('bc-google');
  ...
  if (!d || !d.found) {
    status.textContent =
      'Produit introuvable dans les bases 😕 (fréquent en cosmétique). Tape son nom à la main, ou jette un œil en ligne :';
    if (g) {
      g.href = 'https://www.google.com/search?q=' + encodeURIComponent(code);
  ...
  setTimeout(function () {
    ...
    const nomEl = document.getElementById('pf-nom');
    if (nomEl && d.nom) nomEl.value = cap(d.nom);
    const mqEl = document.getElementById('pf-marque');
    if (mqEl && d.marque) mqEl.value = cap(d.marque);
