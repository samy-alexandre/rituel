  ...
  if (user_id) {
    try {
      ...
      if (SB_URL && SB_KEY) {
        ...
        // Abonnée à Rituel+ ? Alors aucune limite.
        let premium = false;
        try {
          const pr = await fetch(SB_URL + '/rest/v1/profiles?id=eq.' + user_id + '&select=is_premium', { headers });
          if (pr.ok) {
            const rows = await pr.json();
            premium = Array.isArray(rows) && rows.length ? !!rows[0].is_premium : false;
          }
        } catch (e) { /* colonne absente : on applique la limite gratuite */ }
        ...
        if (!premium) {
          ...
          if (rpc.ok) {
            const total = await rpc.json(); // nb d'actions du jour APRÈS incrément
